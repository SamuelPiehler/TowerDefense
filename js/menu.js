...
function printMaps() {
	...
	if (mapsloaded) {
		...
		setTimeout(function () {
			...
			inn.innerHTML =
				`<button onclick="EBI('schwierigkeiten').style.display= 'none';EBI('main').setAttribute('class','contain visible');EBI('start').setAttribute('class','contain invisible')" style="position:absolute; font-size:2em; padding:0px; z-index: 999999999;">&#8592;</button>`
			////console.log(maplist);
	...
	if (saveCode != undefined && saveCode.length >= 11) {
		...
		while (saveCode.length > towerNum * towerDataLength + 11) {
			if (getPrüf2(towerNum * towerDataLength + 29) != getContent(towerNum * towerDataLength + 30)) {
				...
				console.log("prüfziffer = " + getContent(towerNum * towerDataLength + 30) + ", erwartete prüfziffer = " +
					getPrüf2(towerNum * towerDataLength + 29));
				// return;
			}
			towerNum++;
		}
		if (getPrüf1(saveCode.length - 3) != getContent(saveCode.length - 2)) {
			...
			console.log("prüfziffer = " + getContent(saveCode.length - 2) + ", erwartete prüfziffer = " + getPrüf1(saveCode
				.length - 3));
			// return;
		}
		if (getPrüf3(saveCode.length - 2) != getContent(saveCode.length - 1)) {
			console.log("falsche prüfziffer 3 an stelle " + (saveCode.length - 1));
			console.log("prüfziffer = " + getContent(saveCode.length - 1) + ", erwartete prüfziffer = " + getPrüf3(saveCode
				.length - 2));
			// return;
		}
		EBI("menu").style.display = "none";
		...
		function funcAfterMapLoad() {
			...
			while (saveCode.length > towerNum * towerDataLength + 11) {
				...
				tuerme[towerNum] = new Turm(getContent(towerNum * towerDataLength + 12), getContent(towerNum * towerDataLength +
					13), getContent(towerNum * towerDataLength + 10), towerNum, spezialisierung);
				...
				if (tuerme[towerNum].drehGeschw != 0) {
					...
					ladeBild(towertypen[tuerme[towerNum].typ][1], tuerme[towerNum].canvasGeschütz, -tuerme[towerNum].richtung,
						true);
				}
				towerNum++
			}
