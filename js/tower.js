...
function Turm(posx, posy, typ, id, spezialisierung) {
	...
	this.canvasGeschütz.onmouseleave = function () {
		hideRange();
	}; //versteckt reichweite wenn maus nichtmehr über turm
	var id = this.id;
	this.canvasGeschütz.addEventListener('mouseover', function (evt) {
		showRange(evt, this, id);
	}); //zeige reichweite des turms bei hover
	this.canvasGeschütz.addEventListener('click', function () {
		showUpgrade(this, id);
	}); //wenn angeklickt zeige stats und upgrade infos
				...
				tuerme.forEach((item, j) => {
					if (item != undefined && j != this.id) {
						...
						if (entfernung <= this.effektTime * (1 + this.buffStaerken[3] / 100)) {
							if (!(item.effekt[0] == 9)) {
								if (item.typ == 9 && item.buffStaerken[this.effekt[i] - 7] < this.effektStaerke[i] * (1 + this
										.buffStaerken[2] / 100)) {
									item.buffStaerken[this.effekt[i] - 7] = this.effektStaerke[i] * (1 + this.buffStaerken[2] /
									100);
									item.buffTuerme();
								}
								else {
									item.buffStaerken[this.effekt[i] - 7] = Math.max(item.buffStaerken[this.effekt[i] - 7], this
										.effektStaerke[i] * (1 + this.buffStaerken[2] / 100));
								}
							}
							else {
			...
			if (geld >= preis || free) { //genug geld vorhanden?
				...
				for (var i = 0; i < this.effekt.length; i++) { //upgrade towereffeckte
					if (this.effekt[i] == 6) {
						...
						this.effektStaerke[i] = round(this.effektStaerke[i] + towertypen[this.typ][8][this.effekt[i] - 7] * 0.1,
							4);
					}
	...
	target2) { // richte turm in richtung gegner aus grad ist gegnerrichtung target ist gegner id (2 für basic turm stufe 5 zweites target)
		...
		while (this.richtung - grad >
			180) { //setze gradzahlen so dass der unterschied möglichst klein ist (kann immer <= 180 sein)
			grad += 360;
		}
		while (grad - this.richtung > 180) {
			this.richtung += 360;
		}
		if (this.upgradeStufe == maxUpgrade && this.typ == 0 && towertypen[this.typ][12] && target2 !== -
			1) { // wenn basic turm stufe 5
		...
		if (Math.abs(this.richtung - grad) < this.drehGeschw * gameSpeed * tickSpeed * (1 + this.buffStaerken[3] / 100) / this
			...
			if (roundTime - this.letzterAngriff >= 100 * this.angriffsZeit / (1 + this.buffStaerken[1] /
				100)) { //wenn zeit des letzten angriffs länger als angriffszeit her ist
				if (this.typ == 1 && this.upgradeStufe == maxUpgrade && towertypen[this.typ][12]) { //wenn sniper stufe 5
					switch (this.richtung) {
						...
						default:
							...
							.PI); //f(x)=a(x-this.posx)+this.posy funktion der geraden auf der der sniperschuss fliegt   //g(x)=1/a(x-item.posx)+item.posy funktion der gerade gegner und dem nähesten punkt der snipergerade
							if ((this.richtung < 180 && a * (-size / 2 - this.posx) + this.posy < -size / 2) || (this.richtung >
						...
						if (item != undefined) {
							switch (this.richtung) {
								case 0: //wenn der sniper exact nach oben zeigt
									if (this.posy > item.posy && Math.abs(this.posx - item.posx) <= size / 2) {
										if (Math.random() * 100 < this.critChance) {
											item.damage(this.schaden * (1 + this.buffStaerken[0] / 100) * Math.pow(weakenPerHit,
												anzalHits) * this.critDamage, this.effekt.slice(), uebergabeEffektStaerke.slice(),
												uebergabeEffektTime.slice(), this.id, "red");
										}
										else {
											item.damage(this.schaden * (1 + this.buffStaerken[0] / 100) * Math.pow(weakenPerHit, anzalHits),
												this.effekt.slice(), uebergabeEffektStaerke.slice(), uebergabeEffektTime.slice(), this.id);
										}
										anzalHits++;
									}
									break;
								...
								default: //f(x)=a(x-this.posx)+this.posy funktion der geraden auf der der sniperschuss fliegt   //g(x)=1/a(x-item.posx)+item.posy funktion der gerade gegner und dem nähesten punkt der snipergerade
									var xPunktNaheGegner = (item.posy - this.posy + a * this.posx - (1 / a) * item.posx) / (a - 1 /
									a); //f(x)=g(x) ergiebt x
									var yPunktNaheGegner = a * (xPunktNaheGegner - this.posx) + this.posy //x in f(x) ergiebt y
									entfernung = getEntfernung({
										posx: xPunktNaheGegner,
										posy: yPunktNaheGegner
									}, item);
									if (entfernung <= 35) {
										if ((this.richtung < 180 && this.posx > item.posx) || (this.richtung > 180 && this.posx < item
												.posx)) {
											if (Math.random() * 100 < this.critChance) {
												item.damage(this.schaden * (1 + this.buffStaerken[0] / 100) * Math.pow(weakenPerHit,
														anzalHits) * this.critDamage, this.effekt.slice(), uebergabeEffektStaerke.slice(),
												...
												item.damage(this.schaden * (1 + this.buffStaerken[0] / 100) * Math.pow(weakenPerHit,
														anzalHits), this.effekt.slice(), uebergabeEffektStaerke.slice(), uebergabeEffektTime
													.slice(), this.id);
											}
											anzalHits++;
										}
					...
					if (Math.random() * 100 < this.critChance) {
						gegner[target].damage((this.schaden * (1 + this.buffStaerken[0] / 100) + 100 * this.effecktStacks * (1 +
								this.buffStaerken[2] / 100)) * this.critDamage, this.effekt.slice(), uebergabeEffektStaerke.slice(),
							uebergabeEffektTime.slice(), this.id, "red");
					}
					else {
						gegner[target].damage(this.schaden * (1 + this.buffStaerken[0] / 100) + 100 * this.effecktStacks * (1 +
								this.buffStaerken[2] / 100), this.effekt.slice(), uebergabeEffektStaerke.slice(),
							uebergabeEffektTime.slice(), this.id);
					}
					if (target != -1) {
						this.effecktStacks++;
					}
					...
					if (gegner[target] != undefined) {
						...
						if (Math.random() * 100 < this.critChance) { //füge schaden und effeckt auf gegner zu
							...
							gegner[target].damage(this.schaden * (1 + this.buffStaerken[0] / 100), this.effekt.slice(),
								uebergabeEffektStaerke.slice(), uebergabeEffektTime.slice(), this.id);
						}
					}
				}
	...
	.slice(); //berechne übergabewerte für effeckte falls ein angriff ausgeführt wird
			...
			if (item != undefined) {
				...
				if (entfernung <= this.reichweite * (1 + this.buffStaerken[3] / 100)) { //wenn in reichweite
					...
					if (this.drehGeschw == 0) { //drehgeschwindigkeit 0 trifft alle gegner in reichweite und ziehlt nicht
						if (roundTime - this.letzterAngriff >= 100 * this.angriffsZeit / (1 + this.buffStaerken[1] /
							100)) { //wenn letzter angriff langenug her war greife gegner an
							if (Math.random() * 100 < this.critChance) {
								gegner[i].damage(this.schaden * (1 + this.buffStaerken[0] / 100) * this.critDamage, this.effekt
									.slice(), uebergabeEffektStaerke.slice(), uebergabeEffektTime.slice(), this.id, "red");
							}
							else {
								gegner[i].damage(this.schaden * (1 + this.buffStaerken[0] / 100), this.effekt.slice(),
									uebergabeEffektStaerke.slice(), uebergabeEffektTime.slice(), this.id);
							}
							target = -
							2; //zeichen dass gegner angegriffen wurden um angriffszeit zu setzen nachdem alle gegner gehandeld wurden
						}
					}
					else if (this.upgradeStufe == maxUpgrade && this.typ == 0 && towertypen[this.typ][
						12]) { //wenn basic turm stufe 5 (2 targets werden gesucht)
						if (gegner[target] == undefined) { //wähle erstes target
							...
							if ((targetAgro && itemAgro) || (!targetAgro && !itemAgro)) {
								switch (this.targetPrio) {
									case 0:
										if (gegner[target].strecke < item
											.strecke) { //update targets so dass der weiteste gegner immer target 1 ist und der zweit weiteste gegner target 2
											target2 = target;
											target = i;
											...
											targetAgro = itemAgro;
												...
												target2Agro && itemAgro) || (!target2Agro && !itemAgro))) || (itemAgro && !target2Agro)) {
											target2 = i;
										}
										break;
									case 1:
										if (gegner[target].strecke > item
											.strecke) { //update targets so dass der letzte gegner immer target 1 ist und der zweite von hinten  target 2
											target2 = target;
											target = i;
											target2Agro = targetAgro;
											targetAgro = itemAgro;
										...
										break;
									case 2:
										if (gegner[target].leben < item
											.leben) { //update targets so dass der gegner mit den meisten leben immer target 1 ist und der zweite target 2
											target2 = target;
											target = i;
											target2Agro = targetAgro;
											targetAgro = itemAgro;
									...
									case 3:
										if (gegner[target].leben > item
											.leben) { //update targets so dass der gegner mit den wenigsten leben immer target 1 ist und der zweite target 2
											target2 = target;
											target = i;
											target2Agro = targetAgro;
											targetAgro = itemAgro;
										...
										break;
									case 4:
										if (gegner[target].baseSpeed < item
											.baseSpeed) { //update targets so dass der schnellste immer target 1 ist und der zweite target 2
											target2 = target;
											target = i;
											target2Agro = targetAgro;
											targetAgro = itemAgro;
										}
										else if (gegner[target2] == undefined || (gegner[target2].baseSpeed < item.baseSpeed && ((
												target2Agro && itemAgro) || (!target2Agro && !itemAgro))) || (itemAgro && !target2Agro)) {
											target2 = i;
										}
										break;
									case 5:
										if (gegner[target].baseSpeed > item
											.baseSpeed) { //update targets so dass der langsamste gegner immer target 1 ist und der zweite target 2
											target2 = target;
											target = i;
											target2Agro = targetAgro;
											targetAgro = itemAgro;
		...
		if (target > -1) { // wenn ein target gewählt werden konnte
			...
			if (target2 > -1) { //wenn ein 2tes target gewählt wurde
				var entfernung = Math.sqrt(Math.pow((gegner[target2].posx - this.posx), 2) + Math.pow((gegner[target2].posy -
					this.posy), 2));
			...
			var entfernung = Math.sqrt(Math.pow((gegner[target].posx - this.posx), 2) + Math.pow((gegner[target].posy - this
				.posy), 2));
		...
		if (target == -
			...
			if (this.typ == 4 && this.upgradeStufe == maxUpgrade && towertypen[this.typ][12]) {
				if (Math.random() < 0.3) {
					...
					if (gegner[target] != undefined) {
						if (Math.random() * 100 < this.critChance) {
							gegner[target].damage(this.schaden * (1 + this.buffStaerken[2] / 100) * 10 * this.critDamage, [this
								.effekt[0]
							], [this.effektStaerke[0] * 5 * (1 + this.buffStaerken[2] / 100)], [500 * (1 + this.buffStaerken[
								2] / 100)], this.id, "red");
						}
						else {
							gegner[target].damage(this.schaden * (1 + this.buffStaerken[2] / 100) * 10, [this.effekt[0]], [this
								.effektStaerke[0] * 5 * (1 + this.buffStaerken[2] / 100)
							], [500 * (1 + this.buffStaerken[2] / 100)], this.id);
						}
					}
				}
			}
